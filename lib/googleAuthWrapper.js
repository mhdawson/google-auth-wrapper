...
const FAILED_TO_READ_FILE = 1;
const NO_EXISTING_AUTH = 2;
const INVALID_TOKEN = 3;
const FAILED_TO_WRITE_TOKEN = 4;
...
function authorize(storagePath,
  ...
  authCallback(url, function(code, completion) {
    oauthClient.getToken(code, function(err, token) {
      if (err) {
        completion(new Error(INVALID_TOKEN,'Invalid token entered'));
      } else {  
        try {
          fs.writeFile(path.join(storagePath, clientSecrets + '.token'), JSON.stringify(token)); 
          completion();
        } catch (err) {
          completion(new Error(FAILED_TO_WRITE_TOKEN, 'failed to write token to file:' + err));
        }
      }
    });
  });
...
function execute(storagePath, clientSecrets, executeAction) {
  ...
  if (existingToken !== undefined) {
    ...
    throw new Error(NO_EXISTING_AUTH, 'No existing authorization');
  }
...
function createOauthClient(storagePath, clientSecrets) {
  ...
  if (secrets === undefined) { 
    throw new Error(FAILED_TO_READ_FILE, 'failed to read secrets file');
  }
